...
class getProfessionalsReport extends Component {
  constructor(props) {
    ...
    this.getProfessionalsList = this.getProfessionalsList.bind(this)
    this.returnProfessionalsList = this.returnProfessionalsList.bind(this)
  }
...
returnProfessionalsList(){
    if(this.state.professionalsList.length == 0){
      this.getProfessionalsList()
    }
    console.log(this.state.professionalsList)
    return(this.state.professionalsList)
}
...
getProfessionalsList(){
  const apiUrl = `http://${apiHost}:${apiPort}/${apiVer}`;
    ...
    fetch(`${apiUrl}/${collection}?${params}`, {
      ...
      this.setState({"professionalsList":resp}, () => console.log(this.state))
    }).catch(({errors}) => {
      if(errors) {
        let full_error_msg = "";
      ...
      <div className="contentWrapper">
        <div id="divtoPDF">
          <Table className="bordered striped" style={{fontSize:"70%"}}>
            ...
            <tbody>
              {this.returnProfessionalsList().map(function(element,i){
                    return(
                      <tr key={i}>
                        <td>{element.name}</td>
                        <td>{element.cpf}</td>
